...
public class JdbcUserDaoTests extends BaseDaoTests {
    protected static final User USER_1 = new User(1L, "user1", "user1", "ROLE_USER", "email");
    protected static final User USER_2 = new User(2L, "user2", "user2", "ROLE_USER", "email");
    private static final User USER_3 = new User(3L, "user3", "user3", "ROLE_USER", "email");
    ...
    public void create_user_with_null_username() {
        sut.create(null, USER_3.getPassword(), "ROLE_USER", "Email");
    }

    @Test(expected = DataIntegrityViolationException.class)
    public void create_user_with_existing_username() {
        sut.create(USER_1.getUsername(), USER_3.getPassword(), "ROLE_USER", "Email");
    }

    @Test(expected = IllegalArgumentException.class)
    public void create_user_with_null_password() {
        sut.create(USER_3.getUsername(), null, "ROLE_USER", "Email");
    }

    @Test
    public void create_user_creates_a_user() {
        User newUser = new User(-1L, "new", "user", "ROLE_USER", "email");

        boolean userWasCreated = sut.create(newUser.getUsername(), newUser.getPassword(), "ROLE_USER", "Email");
